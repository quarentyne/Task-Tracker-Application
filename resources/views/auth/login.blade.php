<x-guest-layout>
    <div class="flex justify-start pt-10 px-12 pb-8 relative w-full">
        <div class="absolute w-full h-full left-0 bottom-0 bg-[url(../img/login-bg.png)] bg-no-repeat bg-right-bottom z-0"></div>
        <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-5 min-w-[50%] relative z-[1] justify-center">
            @csrf
            <h1 class="text-4xl font-bold font-[Montserrat]">Sign In</h1>
            <div>
                <x-input-with-svg
                    id="email"
            ...
            <div>
                <x-checkbox name="remember">
                    <span class="order-last ml-4">Remember Me</span>
                </x-checkbox>
            </div>
            <div>
            ...
            <div>
                <p>Don’t have an account? <a class="text-sky-600" href="{{ route('register') }}">Create One</a></p>
            </div>
        </form>
    </div>
</x-guest-layout>
